...
import { resolveFiles, useNuxt, defineNuxtModule, addPlugin, addVitePlugin, addWebpackPlugin, addTemplate, findPath, isNuxt3, isIgnored, resolveAlias, addPluginTemplate, logger, nuxtCtx, addComponent, tryResolveModule, installModule, loadNuxtConfig, templateUtils, normalizeTemplate, compileTemplate, normalizePlugin, importModule } from '@nuxt/kit';
...
async function resolveMiddleware() {
  ...
  const middleware = files.map((path) => ({ name: getNameFromPath$1(path), path, global: hasSuffix(path, ".global") }));
...
const componentsModule = defineNuxtModule({
  ...
  setup(componentOptions, nuxt) {
    ...
    nuxt.hook("app:resolve", async () => {
      const allDirs = nuxt.options._layers.map((layer) => normalizeDirs(layer.config.components, layer.cwd)).flat();
...
const version = "3.0.0-27460489.53fbca7";
...
function uniqueBy(arr, uniqueKey) {
...
function watch(nuxt) {
  ...
  const watchHook = debounce((event, path) => nuxt.callHook("builder:watch", event, path));
