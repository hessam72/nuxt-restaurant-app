...
async function _build(nitroContext) {
  ...
  nitroContext.scannedMiddleware = (await Promise.all(serverDirs.map(async (dir) => await scanMiddleware(dir)))).flat().sort((a, b) => b.route.localeCompare(a.route));
...
async function _watch(nitroContext) {
  ...
  const serverDirs = nitroContext._layers.map((layer) => layer.serverDir);
  nitroContext.scannedMiddleware = (await Promise.all(serverDirs.map(async (dir) => await scanMiddleware(dir, (middleware, event) => {
...
function getNitroContext(nuxtOptions, input) {
  const defaults = {
    ...
    _layers: nuxtOptions._layers.map((layer) => ({
      serverDir: resolve(layer.config.srcDir, layer.config.dir?.server || "server")
    }))
  };
